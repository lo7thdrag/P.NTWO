...
uses
  uDBAssetObject, Classes, uDetectedObject, uDBScenario, uDBCubicles,
  uT3Unit;
...
begin
  ...
  if not found then begin
    ...
    if Parent <> nil then begin
      grpID  :=  (Parent as TPlatform_Instance).CubicleGroupID;

      grp := VScenario.CubiclesGroupsList.GetGroupByID(grpID) as T3CubicleGroup;

      if grp <> nil then begin
        if aObject is TT3PlatformInstance then
          detectedObject.TrackNumber := grp.GetTrackNumber(TT3PlatformInstance(aObject).InstanceIndex);
          //TPlatform_Instance(aObject).FData.Platform_Instance_Index)
      end
      else
