...
uses uSimDBEditor,uPlatformDeployment,newclassASTT,
uBaseCoordSystem, ufScenario, uDataModuleTTT;
{$R *.dfm}
...
begin
 ...
 if cbForce.Checked then
  idForce := cbxForce.ItemIndex;

 if cbClassification.Checked then
  idClass := cbxClassification.ItemIndex;

  with SimMgr do
  begin
    Resource_alloc := frmScenario.ResourceAllocation;
