...
uses uBaseCoordSystem, uEnvironmentCharacteristicsWindow, uSimDBEditor, math, uDataModuleTTT,
  uGameAreaList, uExerciseArea, OverbyteIcsWSocket, tttData,
  uFEnvironmentSelection, drawRec, uEnvironment, uGameDefaultSelect,
  uSonarDoubleList,uScrCapture,ufCaptureRes;
...
procedure TfCharacteristicLandmass.ScreenShot(DestBitmap : TBitmap) ;
 ...
 begin
   dmTTT.GetGame_Environment_Definition(StrToInt(EnvironmentForm.envi_id), EnvironmentCharacteristicsWindowForm.GameEnviroDef);
...
begin
  ...
  begin
    ...
    begin
      ...
      edtAverageOceanDepth.Text := FloatToStr(rectSelect.FData.Ave_Ocean_Depth);
      edtShadowZone.Text := FloatToStr(rectSelect.FData.Shadow_Zone_Trans_Loss);
      edtSurface.Text := FloatToStr(rectSelect.FData.Surface_Sound_Speed);
      edtLayer.Text := FloatToStr(rectSelect.FData.Layer_Sound_Speed);
      edtBottom.Text := FloatToStr(rectSelect.FData.Bottom_Sound_Speed);
      ...
      edtAtmospheric.Text := FloatToStr(rectSelect.FData.Atmospheric_Refract_Modifier);
      edtDayVis.Text := FloatToStr(rectSelect.FData.Daytime_Visual_Modifier);
      edtNightVis.Text := FloatToStr(rectSelect.FData.Nighttime_Visual_Modifier);
      edtNightInfra.Text := FloatToStr(rectSelect.FData.Nighttime_Infrared_Modifier);
      edtDayInfra.Text := FloatToStr(rectSelect.FData.Nighttime_Visual_Modifier);
      ...
      edtAirTemp.Text := FloatToStr(rectSelect.FData.Air_Temperature);
      edtBarometricPressure.Text := FloatToStr(rectSelect.FData.Barometric_Pressure);
      edtCloudBaseHeight.Text := FloatToStr(rectSelect.FData.Cloud_Base_Height);
      ...
      //combo box
      cbxShippingRate.Text := IntToStr(rectSelect.FData.Shipping_Rate);
  ...
  begin
    ...
    begin
      ...
      edtSeaState.Text := FloatToStr(Sea_State);
      edtBottomLossCoeff.Text := FloatToStr(Bottomloss_Coefficient);
      edtSurfaceTemperature.Text := FloatToStr(Surface_Temperature);
      edtAverageOceanDepth.Text := FloatToStr(Ave_Ocean_Depth);
      edtShadowZone.Text := FloatToStr(Shadow_Zone_Trans_Loss);
      edtSurface.Text := FloatToStr(Surface_Sound_Speed);
      edtLayer.Text := FloatToStr(Layer_Sound_Speed);
      edtBottom.Text := FloatToStr(Bottom_Sound_Speed);
      ...
      edtAtmospheric.Text := FloatToStr(Atmospheric_Refract_Modifier);
      edtDayVis.Text := FloatToStr(Daytime_Visual_Modifier);
      edtNightVis.Text := FloatToStr(Nighttime_Visual_Modifier);
      edtNightInfra.Text := FloatToStr(Nighttime_Infrared_Modifier);
      edtDayInfra.Text := FloatToStr(Nighttime_Visual_Modifier);
      edtAttenRainRate.Text := FloatToStr(Rain_Rate);
      edtAttenCloud.Text := FloatToStr(Cloud_Attenuation);
      edtAirTemp.Text := FloatToStr(Air_Temperature);
      edtBarometricPressure.Text := FloatToStr(Barometric_Pressure);
      edtCloudBaseHeight.Text := FloatToStr(Cloud_Base_Height);
      ...
      //combo box
      cbxShippingRate.Text := IntToStr(Shipping_Rate);
      ...
      {
      cbHFTransmission.Checked := true; //blu tahu
      }
...
begin
  //sonar
  fSonarDoubleList.fromCharacteristic := true;
  ...
  begin
    ...
    with rectNewObj.FData do
      begin
        ...
        Game_Enviro_Index              := StrToInt(EnvironmentForm.envi_id);
        X_Position_1                   := xx;
        Y_Position_1                   := yy;
        X_Position_2                   := xxx;
        Y_Position_2                   := yyy;
...
begin
  ...
  begin
    dmTTT.GetSubArea_Enviro_Definition(StrToInt(EnvironmentForm.envi_id), rectList);
  end;
end;
...
var
    aGeoset : string;
begin
    getNameArea := uEnvironment.EnvironmentForm.edtArea.Text;
