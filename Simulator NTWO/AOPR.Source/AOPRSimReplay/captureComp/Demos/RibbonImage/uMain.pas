    ...
    begin
      ...
      try
        ...
        begin
          ...
          FFilePathName := DesktopFolder + 'Capture' + IntToStr ( PageControl1.PageCount ) + '.bmp';
...
begin
  ...
  begin
    ...
    FImage := TImage ( TScrollBox ( PageControl1.ActivePage.Controls [ 0 ] ) );
    ...
    begin
      ...
      try
        ...
        FFNE := FFileName + FExtension;
        FFilePathName := FFolder + FFileName + FExtension;
        // If file exists then delete it
        if FileExists ( FFilePathName ) then
          // Prompt user to delete file
          if TaskMessageDlg ( 'Delete', FFilePathName + ' exists, Delete?', mtInformation, [ mbYes, mbNo ], 0 ) = mrYes then
...
begin
  ...
  begin
    ...
    try
      ...
      if Length ( FFolder ) = 0 then
        FFolder := FDefaultDirectory;
      SavePictureDialog1.FileName := JustFilename ( FFilePathName );
      if Length ( SavePictureDialog1.FileName ) = 0 then
        SavePictureDialog1.FileName := '*';
      SavePictureDialog1.InitialDir := FFolder;
      ...
      begin
        ...
        if Length ( FExtension ) = 0 then
          FExtension := '.jpg';
        iTemp := FImage.Picture.Graphic is TJpegImage;
        if not ( iTemp ) and ( FExtension = '.jpg' ) then
        begin

          with TJpegImage ( FImage.Picture.Graphic ) do
            JPEGNeeded;
          ...
          try
            ...
            on EInvalidGraphic do
              TaskMessageDlg ( 'Error', 'Error saving file,' + FFileName, mtWarning, [ mbOk ], 0 );
          end;
