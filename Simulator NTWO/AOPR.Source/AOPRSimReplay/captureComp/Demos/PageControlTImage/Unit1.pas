...
uses Math, ShlObj, ShellAPI, HTMLHelpViewer, uScreenDelay;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          FPath := DesktopFolder + 'Capture' + IntToStr ( PageControl1.PageCount ) + '.bmp';
...
begin
  ...
  Caption := 'Apprehend 6.0 Demo - ' + FPath;
  // The CaptureCount is automatically incremented with each successive capture
  CaptureCount1.Caption := 'Capture Count: ' + IntToStr ( ASGScreenCapture1.CaptureCount );
  ...
  begin
    ...
    try
      ...
      begin
        ...
        iExtension := ExtractFileExt ( SavePictureDialog1.FileName );
        if Length ( iExtension ) = 0 then
          iExtension := '.jpg';
        iTemp := Image.Picture.Graphic is TJpegImage;
        if not ( iTemp ) and ( iExtension = '.jpg' ) then
        begin

          with TJpegImage ( Image.Picture.Graphic ) do
            JPEGNeeded;

          ijp := TJpegImage.Create;
          ijp.PixelFormat := jf24Bit;
          try
            with ijp do
            begin
          ...
          try
            ...
            begin
              ...
              TaskDialog1.Title := 'Error';
              TaskDialog1.Caption := 'Error ';
              TaskDialog1.Text := 'Error saving file, ' + SavePictureDialog1.FileName;
              TaskDialog1.CommonButtons := [ tcbOk ];
              TaskDialog1.Flags := [ tfAllowDialogCancellation ];
              TaskDialog1.MainIcon := tdiError;
              TaskDialog1.Execute;
...
begin
  ...
  begin
    ...
    Image := TImage ( TScrollBox ( PageControl1.ActivePage.Controls [ 0 ] ) );
      ...
      try
        ...
        try
          ...
          begin
            ...
            TaskDialog1.Title := 'Error';
            TaskDialog1.Caption := 'Error ';
            TaskDialog1.Text := 'Error saving file, ' + FPath;
            TaskDialog1.CommonButtons := [ tcbOk ];
            TaskDialog1.Flags := [ tfAllowDialogCancellation ];
            TaskDialog1.MainIcon := tdiError;
            TaskDialog1.Execute;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        try
          Image.Picture.SaveToFile ( IncludeTrailingPathDelimiter ( DesktopFolder ) + ExtractFilename ( FPath ) );
        except
          on EInvalidGraphic do
          begin
            ...
            TaskDialog1.Title := 'Error';
            TaskDialog1.Caption := 'Error ';
            TaskDialog1.Text := 'Error saving file, ' + IncludeTrailingPathDelimiter ( DesktopFolder ) + ExtractFilename ( FPath
              );
            TaskDialog1.CommonButtons := [ tcbOk ];
            TaskDialog1.Flags := [ tfAllowDialogCancellation ];
            TaskDialog1.MainIcon := tdiError;
        ...
        PageControl1.ActivePage.Caption := JustName ( FPath );
        PageControl1.ActivePage.Hint := FPath;
