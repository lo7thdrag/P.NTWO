...
begin
  if (msg.Msg = WM_TIMER) and Connected then begin
    ...
    if (FPacketSize > 0) then begin
      // Complete packet received ?
      if comStat.cbInQue >= FPacketSize then begin
        t := GetTickCount;
        repeat
      ...
      if (FPacketTimeout > 0) and (FFirstByteOfPacketTime <> DWORD(-1)) and
        (GetTickCount - FFirstByteOfPacketTime > FPacketTimeout) then begin
        nRead := 0;
...
procedure Register;
begin
  RegisterComponents('SerialPort', [TCommPortDriver]);
end;
