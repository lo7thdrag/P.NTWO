...
procedure TTCPSocket.DefStrLog(const s: string);
begin
// do nothing
//  LogFile_net(s);
end;

procedure TTCPSocket.HandleException(AE: exception; AClient: TWSocket);
begin
...
procedure TTCPSocket.SetStrLog(const index: Integer; const Value: TGetStrProc);
var x: TGetStrProc;
begin
...
procedure TTCPSocket.RegisterProcedure(const aType: word;
  aProcedure: TRegPacketHandlerProc; const aRecSize: word);
begin
  FRegProcs.RegisterProcedure(aType, aRecSize, aProcedure);
end;

procedure TTCPSocket.UnRegisterProcedure(const aType: word);
begin
  FRegProcs.UnRegisterProcedure(aType);
end;

procedure TTCPSocket.UnregisterAllProcedure;
begin
  FRegProcs.UnregisterALL;

end;

function TTCPSocket.PrepareSendData(const aID: word;
...
procedure TTCPSocket.Thread_Attach;
begin
  WSocket.ThreadAttach;

end;

procedure TTCPSocket.Thread_Detach;
begin
  WSocket.ThreadDetach;
end;


procedure TTCPSocket.ProcessMessages;
begin
...
procedure TTCPSocket.PacketRecognizer(aP: PAnsiChar; const aSize: integer;
...
begin
  ...
  if FRegProcs.IsHandled(recID) then begin
    ...
    FRegProcs[recID].theProc(aP, FRegProcs[pid^.recID].recSize);
  end
  else
    FLogRecv(TimeStr + ': ' + 'UnRegistered ID ' + inttostr(pid^.recID) );
end;
